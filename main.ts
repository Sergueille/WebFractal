...
let framebuffer: WebGLFramebuffer; // Intermediate framebuffer
...
function mainLoop() {
    ...
    if (shaderLoaded("main") && shaderLoaded("blur")) {
        ...
        GL.vertexAttribPointer(
            getShaderAttribute(mainShader, "vPos"),
        ...
        // Send vertex data to shader
        GL.vertexAttribPointer(
            getShaderAttribute(blurShader, "vPos"),
            2, // nb components (here vec2)
            GL.FLOAT, // data type
            false, // normalize
            0, // strinde (how many to skip to reach next data) here 0 is default
...
function CreateRenderTexture(width: number, height: number) {
